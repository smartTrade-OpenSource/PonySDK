...
public class ParserImpl implements Parser {
		...
		if (ServerToClientModel.TYPE_UPDATE.equals(model)) {
			...
			if (lastUpdatedID == newUpdatedID) {
				if (log.isDebugEnabled())
					log.debug("A consecutive update on the same id " + lastUpdatedID
				...
				|| ServerToClientModel.TYPE_REMOVE.equals(model)
				|| ServerToClientModel.TYPE_REMOVE_HANDLER.equals(model)) {
			lastUpdatedID = -1;
		}
	...
	private void parse(final ServerToClientModel model, final String value) {
		if (log.isDebugEnabled())
			log.debug("Writing in the buffer : " + model + " => " + (value != null ? value.length() : 0) + " => "
					+ value);
		final ByteBuffer socketBuffer = buffer.getSocketBuffer();
		socketBuffer.putShort(model.getValue());
