		...
		if (ptObject != null) {
			...
			do {
				...
				if (!BinaryModel.NULL.equals(binaryModel)) {
					if (log.isLoggable(Level.FINE))
						log.log(Level.FINE, "Update : " + binaryModel + " on "
								+ ptObject);
	...
	private void processRemove(final ReaderBuffer buffer, final int objectId) {
		...
		if (ptObject != null) {
			...
			if (parentObject != null) {
				...
				log.warning("Cannot remove PTObject " + ptObject
	...
	public void sendDataToServer(final Widget widget,
		...
		if (log.isLoggable(Level.FINE)) {
			if (widget != null) {
				...
				if (source != null)
					log.fine("Action triggered, Instruction [" + instruction
							+ "] , " + source.getInnerHTML());
			}
		}
