...
public class UIScheduledThreadPoolExecutor implements UIContextListener {
	...
	private void checkUIState() {
		if (UIContext.get() == null)
			throw new IllegalAccessError("UIScheduledThreadPoolExecutor must be called from UI client code");
	}
	...
	public static UIRunnable scheduleAtFixedRate(final Runnable runnable, final long initialDelay, final long period,
			final TimeUnit unit) {
		return INSTANCE.scheduleAtFixedRate0(runnable, initialDelay, period, unit);
	}

	public UIRunnable scheduleAtFixedRate0(final Runnable runnable, final long initialDelay, final long period,
			final TimeUnit unit) {
		checkUIState();
	...
	public static UIRunnable scheduleWithFixedDelay(final Runnable runnable, final long initialDelay, final long delay,
			final TimeUnit unit) {
		return INSTANCE.scheduleWithFixedDelay0(runnable, initialDelay, delay, unit);
	}

	public UIRunnable scheduleWithFixedDelay0(final Runnable runnable, final long initialDelay, final long delay,
			final TimeUnit unit) {
		checkUIState();
