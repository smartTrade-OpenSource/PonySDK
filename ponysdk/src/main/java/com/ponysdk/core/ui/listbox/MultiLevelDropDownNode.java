...
public class MultiLevelDropDownNode<D> {
     ...
     *
     * @param value the value wrapped by the node.
     */
     ...
     * nodes.
     *
     * @param items a collection of items wrapped by the node.
     */
    public MultiLevelDropDownNode(final Collection<ListBoxItem<MultiLevelDropDownNode<D>>> items) {
        if (items == null || items.isEmpty())
            throw new IllegalArgumentException("MultiLevelDropDownNode can't be empty when behaving as a node");
        this.items = items;
        this.value = null;
    }
