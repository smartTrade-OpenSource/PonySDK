...
public abstract class PSuggestOracle extends PObject {
     ...
     * interpretation of the display string depends upon the value of its oracle's. The replacement string is the string that is entered into
     ...
     * display format. In the eventbus that the display of a suggestion exactly matches the input data format,
     ...
     * <p>
     * To access a suggestion's DTO when the suggestion is selected, add a {@link com.ponysdk.core.ui.basic.event.PChangeHandler} to the
     * PSuggestBox (see PSuggestBox's documentation for more information). In the
     * <code>SuggestionHandler.onSuggestionSelected(PSuggestionEvent eventbus)</code> method, obtain the selected
     * <code>Suggestion</code> object from the {@link com.ponysdk.core.ui.basic.event.PChangeHandler} object, and downcast the
