...
org.eclipse.jetty.websocket.servlet.WebSocketServlet {
	...
	private final BlockingQueue<Buffer> buffers = new ArrayBlockingQueue<>(
			NUMBER_OF_BUFFERS);
	...
	public void init() throws ServletException {
		...
		applicationManager = (AbstractApplicationManager) getServletContext()
				.getAttribute(
						AbstractApplicationManager.class.getCanonicalName());
		...
		if (log.isInfoEnabled())
			log.info("Buffer allocation initialized {}", DEFAULT_BUFFER_SIZE
					* buffers.size());
	}
	...
	public void configure(final WebSocketServletFactory factory) {
		...
		factory.setCreator((request, response) -> new WebSocket(request,
				response));
	}
	...
	public class WebSocket implements WebSocketListener {
		...
		WebSocket(final ServletUpgradeRequest request,
		...
		public void onWebSocketConnect(final Session session) {
			if (log.isInfoEnabled())
				log.info("WebSocket connected from {}, sessionID {}", session
						.getRemoteAddress(), request.getSession().getId());
			...
			Application application = SessionManager.get().getApplication(
					sessionId);
			if (application == null) {
				application = new Application(request.getSession(),
						applicationManager.getOptions(),
						UserAgent.parseUserAgentString(request
			...
			try {
				...
				socketBuffer.putShort(ServerToClientModel.UI_CONTEXT_ID
						.getValue());
		...
		public void onWebSocketText(final String text) {
			if (context.getUIContext().isLiving()) {
				...
				try {
					...
					if (!ClientToServerModel.HEARTBEAT.toStringValue().equals(
							text)) {
						...
						applicationManager.fireInstructions(
								context.getJsonObject(), context);
					} else {
						if (log.isDebugEnabled())
							log.debug("Heartbeat received from terminal");
					}
				} catch (final Throwable e) {
					log.error("Cannot process message from the browser: {}",
							text, e);
				} finally {
					if (monitor != null)
		 ...
		 */
		@Override
		public void onWebSocketBinary(final byte[] arg0, final int arg1,
				final int arg2) {
		...
		public void flush(final Buffer buffer) {
			if (buffer != null) {
				if (context.getUIContext().isLiving()) {
					if (isSessionOpen()) {
						try {
							final ByteBuffer socketBuffer = buffer
									.getSocketBuffer();
		...
		private void flush(final ByteBuffer socketBuffer) {
			if (socketBuffer.position() != 0) {
				if (monitor != null)
					monitor.onBeforeFlush(WebSocket.this,
							socketBuffer.position());
				socketBuffer.flip();
				try {
					final Future<Void> sendBytesByFuture = session.getRemote()
							.sendBytesByFuture(socketBuffer);
