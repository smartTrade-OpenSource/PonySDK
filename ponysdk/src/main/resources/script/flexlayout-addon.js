(function () {
  ...
  AbstractAddon.defineAddon("com.ponysdk.core.ui.flexlayout.FlexLayoutAddon", {
    ...
    init: function () {
      ...
      this._iconMode = 'text';       // 'text' or 'attribute'
    ...
    _createFactory: function () {
      ...
      return function (tabNode) {
        ...
        placeholder.className = 'fl-tab-placeholder';
    ...
    addTab: function (tabId, tabName, widgetId, tabsetId, icon) {
    ...
    addPinnedTab: function (tabId, tabName, widgetId, tabsetId, icon) {
      if (!this._model) return;
      this._tabWidgetMap[tabId] = widgetId;
      var tabDef = { id: tabId, name: tabName, component: 'pwidget', config: { widgetId: widgetId }, enableClose: false, enableDrag: false };
      if (icon) tabDef.icon = icon;
      this._model.doAction({
        type: 'ADD_TAB', tabsetId: tabsetId || null, select: true,
        tab: tabDef
     ...
     */
    setIconMode: function (mode, attributeName) {
      this._iconMode = mode || 'text';
      this._iconAttribute = attributeName || 'ui-icon';
      // Propagate to the layout instance for sidebar/tab rendering
      if (this._layout) {
        this._layout.setIconMode(this._iconMode, this._iconAttribute);
      }
    },
    ...
    _popOutToFloat: function (tabId, info, x, y, w, h) {
      ...
      // The window lives in <body>, outside the layout container, so it inherits nothing from the
      // cascade: carry the skin classes over or it falls back to the default palette.
      if (this._layout && this._layout.getSkinClasses) {
        this._layout.getSkinClasses().forEach(function (c) { win.classList.add(c); });
      }
      win.dataset.tabId = tabId;
      // Geometry only: the looks live in .fl-popout-window so a skin can reach them.
      win.style.cssText = 'left:' + (x || 100) + 'px;top:' + (y || 100) + 'px;width:' + (w || 400) + 'px;height:' + (h || 300) + 'px;';

      var titleBar = document.createElement('div');
      titleBar.className = 'fl-popout-titlebar';
      var titleSpan = document.createElement('span');
      titleSpan.className = 'fl-popout-title';
