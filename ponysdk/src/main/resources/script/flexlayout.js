...
(function (global, document) {
  ...
  class Layout {
    constructor(opts) {
      ...
      // Icon rendering mode: 'text' (default) or 'attribute'
      this._iconMode = 'text';
      this._iconAttribute = 'ui-icon';
     ...
     * @param mode 'text' (default) or 'attribute'
     * @param attributeName HTML attribute name for 'attribute' mode (default: 'ui-icon')
     */
    setIconMode(mode, attributeName) {
      this._iconMode = mode || 'text';
      this._iconAttribute = attributeName || 'ui-icon';
      this._render(); // Re-render to apply new icon mode
    }
    setLocked(v) {
      this._locked = !!v;
    ...
    /**
     * Renders an icon into a container element based on the current icon mode.
     * @param iconKey the icon key/name
     * @param container the DOM element to render into
     */
    _renderIcon(iconKey, container) {
      if (this._iconMode === 'attribute') {
        container.setAttribute(this._iconAttribute, iconKey);
      } else {
        container.textContent = iconKey;
      }
    }
    ...
    _showBorderContextMenu(tab, border, x, y) {
      ...
      this.getSkinClasses().forEach(c => menu.classList.add(c));
      // Only the position is inline: everything else lives in .fl-border-context-menu so a
      // skin can restyle it (inline declarations beat any stylesheet).
      ...
      items.forEach(item => {
        ...
        el.className = 'fl-context-item'; // hover handled by CSS, not an inline background
        ...
        openBorders.forEach((ob, oIdx) => {
          ...
          if (oIdx < openBorders.length - 1) {
            ...
            splitter.addEventListener('pointerdown', ev => {
              ...
              this._beginResize(splitter, false);
              const prevPane = pane;
        ...
        handle.addEventListener('pointerdown', e => {
          ...
          const onUp = () => {
            ...
            if (finalSize < SNAP_THRESHOLD) {
              ...
              // The panel and the layout insets were updated inline as the pointer moved, so the
              // rendered state already matches the model: emitting 'change' would only rebuild it.
              if (this.onModelChange) this.onModelChange(this.model);
            }
          };
    ...
    /**
     * Pins the resize cursor on the whole layout for the duration of a drag and freezes the
     * content underneath. Without it the cursor is recomputed from whatever element the pointer
     * happens to be over, so it flickered on every move and on every re-render.
     * Every resize handle — pane splitter, sidebar edge, sidebar zone splitter — goes through this.
     */
    /**
     * The classes on the container that carry the skin: everything except our own structural and
     * transient state classes. Overlays are re-parented to <body>, outside the container, so they
     * do not inherit the theme through the cascade and must be tagged explicitly.
     */
    getSkinClasses() {
      const structural = new Set(['fl-layout', 'fl-resizing', 'fl-resizing-col', 'fl-resizing-row',
                                  'fl-has-maximized', 'fl-layout-locked']);
      return [...this.container.classList].filter(c => !structural.has(c));
    }

    _beginResize(handleEl, isH) {
      this.container.classList.add('fl-resizing', isH ? 'fl-resizing-col' : 'fl-resizing-row');
      if (handleEl) handleEl.classList.add('fl-sp-active');
      // Belt for pointer excursions outside the container while captured.
      document.body.style.cursor = isH ? 'col-resize' : 'row-resize';
      document.body.style.userSelect = 'none';
    }

    _endResize(handleEl) {
      this.container.classList.remove('fl-resizing', 'fl-resizing-col', 'fl-resizing-row');
      if (handleEl) handleEl.classList.remove('fl-sp-active');
    ...
    _mkTabBtn(tab, tabset, idx) {
      ...
      if (tab.getIcon()) {
        const ic = document.createElement('span');
        ic.className = 'fl-tab-icon'; this._renderIcon(tab.getIcon(), ic);
        btn.appendChild(ic);
      }
      const lbl = document.createElement('span');
    ...
    _onDragMove(ev) {
      ...
      if (!d.isStarted) {
        ...
        if (!d.isNew && d.tab.getIcon()) {
          const ic = document.createElement('span');
          ic.className = 'fl-tab-icon'; this._renderIcon(d.tab.getIcon(), ic);
    ...
    _updateDrop(ev) {
      ...
      if (hit.location === 'insert') {
        ...
        const b = hit.bar;
        ind.classList.add('fl-drop-bar');
        ind.style.cssText = `display:block;left:${b.x - 1.5}px;top:${b.top}px;height:${b.height}px;`;
        return;
      }
      ...
      ind.classList.remove('fl-drop-bar');
      ind.style.cssText = `display:block;left:${L}px;top:${T}px;width:${W}px;height:${H}px;`;
    }
    ...
    _onResUp() {
      ...
      this._endResize(r.sp);
      // The weights are already on the model and the flex styles already applied, so emitting
      // 'change' here would only tear the whole layout down and rebuild it identically —
      // destroying every tab content, hence every hosted widget, to commit two numbers.
      if (this.onModelChange) this.onModelChange(this.model);
    }
  }
