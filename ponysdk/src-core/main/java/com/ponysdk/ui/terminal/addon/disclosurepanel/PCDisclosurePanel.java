...
public final class PCDisclosurePanel extends Composite implements HasWidgets.ForIsWidget, HasAnimation, HasOpenHandlers<PCDisclosurePanel>, HasCloseHandlers<PCDisclosurePanel> {
    ...
    private static class ContentAnimation extends Animation {
        ...
        /**
         * The {@link PCDisclosurePanel} being affected.
         */
        private PCDisclosurePanel curPanel;
         ...
         */
        public void setOpen(PCDisclosurePanel panel, boolean animate) {
        ...
        protected void onUpdate(double progress) {
            final int scrollHeight = DOM.getElementPropertyInt(curPanel.contentWrapper.getElement(), "scrollHeight");
    ...
    @Override
    public HandlerRegistration addOpenHandler(OpenHandler<PCDisclosurePanel> handler) {
        return addHandler(handler, OpenEvent.getType());
    }

    /**
     * The default header widget used within a {@link PCDisclosurePanel}.
     */
    private class DefaultHeader extends Widget implements HasText, OpenHandler<PCDisclosurePanel>, CloseHandler<PCDisclosurePanel> {
        ...
        private DefaultHeader(Imager imager, String text) {
            ...
            imager.updateImage(isOpen, iconImagePanel);
            DOM.setStyleAttribute(imageTD, "width", iconImagePanel.getOffsetWidth() + "px");

            DOM.appendChild(imageTD, iconImagePanel.getElement());
        ...
        private DefaultHeader(final Image openImage, final Image closedImage, String text) {
            this(new Imager() {

                @Override
                public void updateImage(boolean open, SimplePanel imagePanel) {
                    if (open) {
                        imagePanel.setWidget(openImage);
                        openImage.getElement().setAttribute("border", "0");
                    } else {
                        imagePanel.setWidget(closedImage);
                        closedImage.getElement().setAttribute("border", "0");
                    }
                }
            }, text);
        }
        ...
        @Override
        public final void onClose(CloseEvent<PCDisclosurePanel> event) {
            setStyle();
        }

        @Override
        public final void onOpen(OpenEvent<PCDisclosurePanel> event) {
            setStyle();
        }
     ...
     */
    public PCDisclosurePanel(Image openImage, Image closedImage, String headerText) {
        this();
    ...
    @Override
    public HandlerRegistration addCloseHandler(CloseHandler<PCDisclosurePanel> handler) {
    ...
    public HasText getHeaderTextAccessor() {
        final Widget widget = header.getWidget();
    ...
    private static class WidgetIterators {
